...
try {
  ...
  const workspace = async (token, input) => {
    const result = await api(token,"/api/task-workspace",input);
    assert.equal(result.ok,true,result.error);return result.data;
  };
  const captureId=randomUUID();
  await workspace(owner.token,{action:"capture",id:captureId,text:"Pack for camp"});
  await workspace(owner.token,{action:"capture",id:captureId,text:"Pack for camp"});
  assert.equal((await workspace(owner.token)).tasks.filter(t=>t.id===captureId).length,1,"capture retries do not duplicate tasks");
  assert.ok(!items(await planner(owner.token,currentMonday)).some(t=>t.id===captureId),"backlog stays outside the weekly planner");
  const resource={itemId:captureId};
  await workspace(owner.token,{action:"task",resource,text:"Pack for overnight camp"});
  assert.equal((await workspace(owner.token)).tasks.find(t=>t.id===captureId).text,"Pack for overnight camp","backlog task names can be corrected");
  assert.equal((await api(viewer.token,"/api/task-workspace",{action:"task",resource,claim:true})).ok,false,"viewers cannot claim tasks");
  assert.equal((await api(outsider.token,`/api/task-workspace?itemId=${captureId}`)).ok,false,"task discussion is household private");
  assert.equal((await api(owner.token,"/api/task-workspace",{action:"task",resource,responsibleMemberId:outsider.userId})).ok,false,"foreign responsibility rejected");
  await workspace(owner.token,{action:"task",resource,responsibleMemberId:partner.userId,deadline:addDays(week,4)});
  await workspace(owner.token,{action:"task",resource,responsibleMemberId:partner.userId});
  assert.equal((await client.query("select count(*)::int as count from notification_outbox where user_id=$1 and title='A task is assigned to you'",[partner.userId])).rows[0].count,1,"assignment sends exactly one notification on change");
  assert.equal((await api(owner.token,"/api/task-workspace",{action:"task",resource,claim:true})).ok,false,"claim cannot steal an assigned task");
  await workspace(owner.token,{action:"task",resource,responsibleMemberId:null});
  const claims=await Promise.all([api(owner.token,"/api/task-workspace",{action:"task",resource,claim:true}),api(partner.token,"/api/task-workspace",{action:"task",resource,claim:true})]);
  assert.equal(claims.filter(r=>r.ok).length,1,"concurrent claims have a single winner");
  await workspace(owner.token,{action:"task",resource,isBacklog:false,planningDate:addDays(week,2)});
  assert.equal(items(await planner(owner.token)).find(t=>t.id===captureId)?.deadline,addDays(week,4),"scheduling keeps deadline separate");
  await workspace(owner.token,{action:"task",resource,isBacklog:true});
  await client.query("update planning_items set week_start_date=$2::date where id=$1",[captureId,addDays(currentMonday,-14)]);
  await client.query("select carry_over_open_tasks($1,$2::date,$3::date,now())",[householdIds[0],today,currentMonday]);
  assert.equal((await workspace(owner.token)).tasks.find(t=>t.id===captureId).weekStartDate,addDays(currentMonday,-14),"automatic carryover leaves backlog untouched");
  const stepId=randomUUID(),commentId=randomUUID(),fileId=randomUUID();
  await workspace(owner.token,{action:"add",resource,id:stepId,kind:"checklist",text:"Pack towels"});
  await workspace(partner.token,{action:"check",resource,id:stepId,completed:true});
  await workspace(partner.token,{action:"add",resource,id:commentId,kind:"comment",text:"I’ll bring the towels"});
  await workspace(owner.token,{action:"add",resource,id:fileId,kind:"file",text:"school.pdf",fileData:Buffer.from("%PDF-1.4\nTest fixture\n").toString("base64")});
  const details=await api(viewer.token,`/api/task-workspace?itemId=${captureId}`);
  assert.equal(details.data.entries.length,3);assert.equal(details.data.entries.find(e=>e.id===stepId).completed,true);
  assert.equal(details.data.entries.find(e=>e.id===commentId).author,"Sam");
  const downloaded=await fetch(`${baseURL}/api/task-workspace?file=${fileId}`,{headers:{Authorization:`Bearer ${partner.token}`}});
  assert.equal(downloaded.status,200);assert.equal(await downloaded.text(),"%PDF-1.4\nTest fixture\n");
  assert.ok(downloaded.headers.get("content-disposition").startsWith("attachment"));
  assert.equal((await api(outsider.token,`/api/task-workspace?file=${fileId}`)).ok,false,"attachments cannot cross households");
  const eventResource={calendarId:sharedCalendar,eventId:"google-readonly-occurrence"};
  await workspace(partner.token,{action:"add",resource:eventResource,id:randomUUID(),kind:"comment",text:"Meet by the gate"});
  assert.equal((await api(owner.token,`/api/task-workspace?calendarId=${sharedCalendar}&eventId=google-readonly-occurrence`)).data.entries.length,1,"Google events support local discussion");
  assert.equal((await api(owner.token,`/api/task-workspace?calendarId=${sharedCalendar}&eventId=different-occurrence`)).data.entries.length,0,"event discussion applies to one occurrence");
  assert.equal((await api(partner.token,"/api/task-workspace",{action:"add",resource:{calendarId:privateCalendar,eventId:"private"},id:randomUUID(),kind:"comment",text:"No"})).ok,false,"private Google events keep their privacy boundary");
