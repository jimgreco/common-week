...
enum PreviewData {
    static let user = SessionIdentity(userId: "demo-jim", email: "jim@example.com", displayName: "Jim", householdId: "demo-household", role: "owner")
    ...
    static func planner(weekStart: String) -> WeeklyPlannerData {
        ...
        let days = (0..<7).map { index -> DayPlan in
            ...
            let weather = DailyWeather(date: date, locationId: location.id, conditionCode: index == 2 || index == 5 ? 61 : 0, highF: Double(82 - index), lowF: Double(66 + index % 3), precipitationProbability: index == 5 ? 72 : 8 + index * 3, precipitationAmount: index == 5 ? 0.18 : 0, windSpeedMph: 8, sunrise: WeekDate.iso8601.string(from: start), sunset: WeekDate.iso8601.string(from: start), hourly: [], status: "available", errorMessage: nil)
            return DayPlan(date: date, location: location, weather: weather, events: [event], items: items)
        }
        return WeeklyPlannerData(
            ...
            editableCalendars: [EditableCalendar(id: "calendar-family", name: "Family", color: "#688173", sectionGroup: "critical")],
